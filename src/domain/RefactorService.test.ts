...
describe("RefactorService (Domain Link Refactoring)", () => {
  it("replaces a single Wiki-link with the new title", () => {
    ...
    const result = RefactorService.refactorWikiLinks(markdown, "Cardiology", "Cardiovascular Medicine");
...
Also see [[Neurology]].
`;
    const result = RefactorService.refactorWikiLinks(markdown, "Cardiology", "Cardio");
  ...
  it("preserves link alias while updating the target title", () => {
    ...
    const result = RefactorService.refactorWikiLinks(markdown, "Cardiology", "Cardiovascular");
